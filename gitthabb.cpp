...
int main()
{
    ...
    t1.loadFromFile("pic/cat.jpeg");
    t2.loadFromFile("pic/name.png");
    t3.loadFromFile("pic/st.png");
    t4.loadFromFile("pic/op.png");
    t5.loadFromFile("pic/ex.png");
    t6.loadFromFile("pic/png-klev-club-qkys-p-knopka-nazad-png-15-Photoroom.png");
    ...
    Font font;
    if (!font.openFromFile("timesnewromanps_italicmt.ttf"))
    {
        cout << "Ошибка!\n";
        return 1;
    }
    ...
    while (window.isOpen())
    {
        ...
        if (!mash){
            ...
            float ballRight  = ball.getPosition().x + 2*ball.getRadius(); // тяжелая артиллерия чтобы точно не проходила на сквозь
